  ...
  GPIO_InitTypeDef  GPIO_InitStructure;	
    ...
    GPIO_Init(GPIOB, &GPIO_InitStructure);//初始化
	  ...
	  GPIO_Init(GPIOF, &GPIO_InitStructure);//初始化			
		...
		if(1)return 0;//已经校准							//返回值：1，成功获取数据
																					//        0，获取失败，要重新校准
		else			   //未校准
		{ 										    
			LCD_Clear(WHITE);//清屏
...
//	 	key=KEY_Scan(0);
		...
		{	
			...
			{	
				...
				else  if(tp_dev.x[0]<(180)&& tp_dev.x[0]>(60)&& tp_dev.y[0]>(190) && tp_dev.y[0]<230 ) 
				{
//					safe_ok=1;
				}
				
				else if(tp_dev.x[0]<(180)&& tp_dev.x[0]>(60)&& tp_dev.y[0]>(240) && tp_dev.y[0]<280 ) 
				{
					motor_flash(0,0);
				}
...
//		LCD_Fill(60+2,200+2,180-2,240-2,CYAN);
	}
}	
