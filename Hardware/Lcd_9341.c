...
//size:字体大小 16/32
//mode:叠加方式(1)还是非叠加方式(0)
void LCD_ShowFont(u16 x,u16 y,u8 num,u8 size,u8 mode)
{  							  
  u8 temp,t1,t;
	u16 y0=y;
	u8 csize;
		if(size==16){csize=32;}
		else if(size==32){csize=128;}

	for(t=0;t<csize;t++)
	{  
		if(size==16){temp=hz_1616[num][t];}
		else if(size==32){temp=hz_3232[num][t];}
		else return;	
		for(t1=0;t1<8;t1++)
		{			    
			if(temp&0x80)LCD_Fast_DrawPoint(x,y,POINT_COLOR);
...
void Show_Head(int x,int y)
{
	...
	POINT_COLOR=BLACK;   
		...
		for(i=0;i<6;i++)
	{
	LCD_ShowFont(x+32*i,y,i,32,1);
	}
	for(i=0;i<5;i++)
	{
		LCD_ShowFont(x+50+16*i,y+40,i,16,1);
	}

	LCD_Fill(60,190,180,230,BLACK);
	
	LCD_Fill(60+2,190+2,180-2,230-2,CYAN);
	
	LCD_ShowString(100,202,50,16,16,"test");
	
	LCD_Fill(60,240,180,280,BLACK);
	
	LCD_Fill(60+2,240+2,180-2,280-2,CYAN);
	
	LCD_ShowString(100,252,50,16,16,"stop");
	
	LCD_ShowString(10,300,200,16,16,"Copy Right");	
	LCD_ShowString(155,300,200,16,16,"2015.10.31");	
}
